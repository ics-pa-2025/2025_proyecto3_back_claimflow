...
import {
  EstadoProyecto,
  EstadoProyectoDocument,
} from './schemas/estado-proyecto.schema';
...
export class EstadoProyectoService {
  constructor(
    @InjectModel(EstadoProyecto.name)
    private estadoProyectoModel: Model<EstadoProyectoDocument>,
  ) {}

  async create(
    createEstadoProyectoDto: CreateEstadoProyectoDto,
  ): Promise<EstadoProyecto> {
    const createdEstadoProyecto = new this.estadoProyectoModel(
      createEstadoProyectoDto,
    );
  ...
  async findOne(id: string): Promise<EstadoProyecto> {
    const estadoProyecto = await this.estadoProyectoModel
  ...
  async update(
    id: string,
    updateEstadoProyectoDto: UpdateEstadoProyectoDto,
  ): Promise<EstadoProyecto> {
    const updatedEstadoProyecto = await this.estadoProyectoModel
      .findOneAndUpdate({ _id: id, deletedAt: null }, updateEstadoProyectoDto, {
        new: true,
      })
      .exec();
