...
export class TipoProyectoService {
  ...
  async create(
    createTipoProyectoDto: CreateTipoProyectoDto,
  ): Promise<TipoProyecto> {
    const createdTipoProyecto = new this.tipoProyectoModel(
      createTipoProyectoDto,
    );
  ...
  async findOne(id: string): Promise<TipoProyecto> {
    const tipoProyecto = await this.tipoProyectoModel
  ...
  async update(
    ...
    const updatedTipoProyecto = await this.tipoProyectoModel
      .findOneAndUpdate({ _id: id, deletedAt: null }, updateTipoProyectoDto, {
        new: true,
      })
      .exec();
